...
// OAuth providers are gated on a build-time env var. If neither is enabled,
// the entire OAuth section is hidden — preventing the dead-button UX where
// clicking Google / Apple just spins forever because GoTrue's external
// provider isn't configured.
const GOOGLE_OAUTH_ENABLED = process.env.NEXT_PUBLIC_ENABLE_GOOGLE_OAUTH === 'true';
const APPLE_OAUTH_ENABLED = process.env.NEXT_PUBLIC_ENABLE_APPLE_OAUTH === 'true';
const ANY_OAUTH_ENABLED = GOOGLE_OAUTH_ENABLED || APPLE_OAUTH_ENABLED;
...
function OAuthButton({ provider, label, icon }: { provider: 'google' | 'apple'; label: string; icon: React.ReactNode }) {
  ...
  async function handleClick() {
    ...
    // If GoTrue isn't configured for this provider, surface a friendly toast
    // instead of leaving the spinner stuck forever.
    if (error) {
      toast(t('oauthUnavailable', { provider: label }), 'error');
      setLoading(false);
    }
    // On success the browser is already navigating to the IdP — no need to
    // clear loading, it'll unmount on redirect.
  }
  return (
    <motion.button
...
function MagicLinkForm({ t }: { t: ReturnType<typeof useTranslations> }) {
  ...
  async function onSubmit({ email }: MagicForm) {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email,
      // Redirect users back to whichever locale they were using when they
      // submitted the form (en/ar) — they shouldn't get yanked to /en just
      // because that was hardcoded.
      options: { emailRedirectTo: `${window.location.origin}/${locale}`, shouldCreateUser: true },
    });
        ...
        <div className="rounded-3xl border border-gold/25 bg-surface/75 backdrop-blur-xl shadow-popLg p-5 sm:p-8 space-y-7">
          ...
          {ANY_OAUTH_ENABLED && (
            <>
              ...
              {/* OAuth buttons (only those configured at build time) */}
              <motion.div
                initial={{ opacity: 0, y: 8 }}
                animate={{ opacity: 1, y: 0 }}
